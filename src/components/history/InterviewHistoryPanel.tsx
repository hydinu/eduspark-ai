    ...
    return (
      <Card className="p-12 text-center border-dashed">
        ...
        <p className="text-sm text-muted-foreground mb-5">
          Practice a mock interview to start building your record.
        </p>
        <Link to="/interview">
          <Button>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Sessions", value: sessions.length, icon: Mic, color: "text-primary" },
          {
            ...
            value: `${avgScore}/100`,
            icon: TrendingUp,
            color: avgScore >= 70 ? "text-emerald-500" : "text-amber-500",
          },
      ...
      <div className="space-y-2">
        ...
        {sessions.map((s: any) => {
          ...
          return (
            <Card
              key={s.id}
              className={cn("overflow-hidden border transition-all", isOpen && "shadow-soft")}
            >
              <button
              ...
              >
                ...
                <div className="text-right shrink-0">
                  <Badge
                    variant={
                      s.score >= 80 ? "default" : s.score >= 50 ? "secondary" : "destructive"
                    }
                    className="text-xs"
                  >
                ...
                <div className="border-t bg-secondary/20 p-4 space-y-4">
                  {s.feedback && (
                    <div>
                      ...
                      <div className="bg-card rounded-lg p-4 text-sm whitespace-pre-wrap border leading-relaxed">
                        {s.feedback}
                      </div>
                    </div>
                  )}
                      ...
                      <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                        {transcript.map((t, i) => (
                          <div
                            ...
                            className={cn(
                              "flex gap-2",
                              t.role === "candidate" ? "flex-row-reverse" : "",
                            )}
                          >
                            <div
                              className={cn(
