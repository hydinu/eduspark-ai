...
export function QuizHistoryPanel({ onClose }: { onClose?: () => void }) {
  ...
  const avgPct = attempts?.length
    ? Math.round(
        attempts.reduce((a, b) => a + Math.round((b.score / b.total) * 100), 0) / attempts.length,
      )
    : 0;
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Total Attempts", value: attempts.length, icon: Brain, color: "text-primary" },
          {
            ...
            value: `${avgPct}%`,
            icon: TrendingUp,
            color: avgPct >= 70 ? "text-emerald-500" : "text-amber-500",
          },
                      ...
                      >
                        ...
                        <div className="flex gap-2 flex-wrap text-xs">
                          ...
                          >
                            {correct
                              ? "✓ Correct"
                              : `✗ You: ${q.options?.[userAns] ?? "Unanswered"}`}
                          </span>
                          {!correct && (
