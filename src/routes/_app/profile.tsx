...
function ProfilePage() {
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        ...
        avgInterview: iv.length
          ? Math.round(iv.reduce((a, b) => a + (b.score ?? 0), 0) / iv.length)
          : 0,
  ...
  const skillsList = resume?.skills
    ? Object.values(resume.skills as Record<string, string[]>).flat()
    : [];
  const projectCount = Array.isArray(resume?.projects)
    ? (resume.projects as any[]).filter((p) => p.title).length
    : 0;
    ...
    <div className="p-4 sm:p-6 lg:p-10 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
        <StatCard
          ...
          sub={`${stats?.quizAttempts ?? 0} attempts`}
          color="text-primary"
        />
        <StatCard
          icon={Mic}
          label="Interviews"
          value={String(stats?.interviewCount ?? 0)}
          sub={`Avg ${stats?.avgInterview ?? 0}/100`}
        ...
        <StatCard
          ...
          label="Courses"
          value={String(
            (stats?.coursesSaved ?? 0) + (stats?.coursesActive ?? 0) + (stats?.coursesDone ?? 0),
          )}
          sub={`${stats?.coursesDone ?? 0} completed`}
          color="text-success"
        />
        <StatCard
          icon={TrendingUp}
          label="AI Chats"
          value={String(stats?.chatCount ?? 0)}
          sub="Conversations"
          color="text-warning"
        />
      </div>
                ...
                .map((p, i) => (
                  <div key={i} className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      {p.tech_stack && (
                        <div className="text-xs text-muted-foreground">{p.tech_stack}</div>
                      )}
                    </div>
                  </div>
                ))}
      ...
      <Card className="p-5 mb-6">
        ...
        <div className="space-y-4">
          ...
          <ProgressRow
            ...
            value={
              (stats?.coursesDone ?? 0) + (stats?.coursesActive ?? 0) + (stats?.coursesSaved ?? 0) >
              0
                ? Math.round(
                    ((stats?.coursesDone ?? 0) /
                      ((stats?.coursesDone ?? 0) +
                        (stats?.coursesActive ?? 0) +
                        (stats?.coursesSaved ?? 0))) *
                      100,
                  )
                : 0
            }
    ...
    <div>
      <div className="flex justify-between text-sm mb-1">
        ...
        <span
          className={cn(
            "font-bold",
            value >= 70 ? "text-success" : value >= 40 ? "text-warning" : "text-muted-foreground",
          )}
        >
          {value}%
        </span>
