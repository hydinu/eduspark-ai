...
async def extract_with_playwright(url: str) -> str:
    """Use Playwright/Chromium to extract content from dynamic pages."""
    try:
...
def extract_page_content(url: str) -> str:
    """Sync wrapper — tries Playwright first, falls back to requests."""
    try:
        return asyncio.run(extract_with_playwright(url))
    except RuntimeError:
        # Already in event loop (shouldn't happen in FastAPI sync context)
        return extract_with_requests(url)
    except Exception:
        return extract_with_requests(url)
